...
def Menu(name,url,mode,iconimage,fanart,description,extra,showcontext=True,allinfo={}):
        ...
        if showcontext:
            contextMenu = []
            if showcontext == 'fav':
                contextMenu.append(('Remove from Britcom Central Favorites','XBMC.RunPlugin(%s?mode=105&name=%s)'
                                    %(sys.argv[0], urllib.quote_plus(name))))
            if not name in FAV:
                contextMenu.append(('Add to Britcom Central Favorites','XBMC.RunPlugin(%s?mode=104&name=%s&url=%s&iconimage=%s&fanart=%s&fav_mode=%s)'
...
def getFavorites():
        if os.path.exists(favourites)==False:
            favList = []
            addon_log('Making Favorites File')
            favList.append(('Britcom central Favourites Section','','','','','',''))
            a = open(favourites, "w")
            a.write(json.dumps(favList))
            a.close()        
        else:
