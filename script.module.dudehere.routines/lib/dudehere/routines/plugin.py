...
def save_data(file, data, format='pickle'):
	if format == 'pickle':
		vfs.write_file(file, pickle(data), mode='b')
	else:
		vfs.write_file(file, json.dumps(data))
		
def load_data(file, format='pickle'):
	if format == 'pickle':
		try:
			return unpickle(vfs.read_file(file, mode='b'))
		except Exception, e:
			log(e)
			return None
	else:
		try:
			return json.loads(vfs.read_file(file))
		except Exception, e:
			log(e)
			return None
def log(msg, level=1):
	__log(str(msg).encode("utf-8", "ignore"))
