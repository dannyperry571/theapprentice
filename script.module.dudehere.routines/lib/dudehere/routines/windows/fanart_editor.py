...
class FanartEditor(BaseWindow):
	...
	def onInit(self):
		...
		if plugin.mode == 'update_series_fanart':
			results = fanart.get_all_show_art(plugin.args['tvdb_id'], plugin.args['tmdb_id'], plugin.args['imdb_id'])
		else:
			results = fanart.get_all_movie_art(plugin.args['imdb_id'], plugin.args['tmdb_id'])
	...
	def onClick(self, controlID):
		...
		if controlID in [CONTROLS.POSTERS, CONTROLS.FANARTS]:
			...
			meta = plugin.unpickle(str(meta[0]))
			art = 'cover_url' if self.ART == 'poster' else 'backdrop_url'
			meta[art] = img
			meta = plugin.pickle(meta)
			self.DB.execute("UPDATE metadata_%s SET metadata=? WHERE trakt_id=? " % media, [meta, plugin.args['trakt_id']])
			self.DB.commit()
